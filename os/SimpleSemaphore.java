...
public class SimpleSemaphore
{
    ...
    public synchronized void p() 
    {
        if (value <=0) 
        {
            try 
            { 
            	System.out.println(Thread.currentThread().getName() + " requested a WAIT - NOT AGREED/" + (waiting+1));
                waiting++;
                wait();  // attende
                // waiting--;
                // lo deve fare v()
            ...
            value--;
            System.out.println(Thread.currentThread().getName() + " requested a WAIT - AGREED/" + value);
        }
    } //[m] p
    ...
    public synchronized void v() {
        if (value == maxvalue){
        	System.out.println(Thread.currentThread().getName() + " requested a SIGNAL - NONSENSE");
            // NOP
            return;
            }
        ...
        {
            waiting--;
            System.out.println(Thread.currentThread().getName() + " requested a SIGNAL - AGREED TO WAITING");
            notify(); 
        }
        else{
            value++;
            System.out.println(Thread.currentThread().getName() + " requested a SIGNAL/" + value);
    ...
    /**[m]
     * valore del semaforo
     * @return il valore del semaforo (>=0)
     */
    public int value() 
    { return value; }
